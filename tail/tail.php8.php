...
class Module_tail
{
    ...
    private function doTail($numLines, $file)
    {
        $result = "";

        if (!@file_exists($file))
            return "tail: $file: No such file or directory" . PHP_EOL;

        if (!@is_readable($file))
            return "tail: $file: Permission denied" . PHP_EOL;

        if (@is_dir($file))
            return "tail: $file: Is a directory" . PHP_EOL;

        if (!$this->isTextFile($file))
            return "tail: '$file': Is not ASCII file" . PHP_EOL;

        $handle = @fopen($file, "r");
        if ($handle === false)
            return "tail: '$file': Can't read it" . PHP_EOL;
    ...
    public function execute($args)
    {
        try
        {
            ...
            foreach ($files as $file)
            {
                ...
                $result .= $this->doTail($num_lines, $file) . PHP_EOL;
            }
