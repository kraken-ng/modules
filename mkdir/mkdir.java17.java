...
public class Module_mkdir
{
    ...
    private String[] doMkdir(String[] filepaths)
    {
        ...
        for (String filepath : filepaths)
        {
            try
            {
                ...
                File input_file = new File(filepath).getAbsoluteFile();
                
                if (input_file.exists() == true)
                    throw new Exception("mkdir: dir '" + filepath + "': already exists");
            
                if (input_file.mkdirs() == false)
                    throw new Exception("mkdir: dir '" + filepath + "': failed");
                
                result += "Directory: '" + filepath + "' created" + JAVA_EOL;
            }
            catch(Exception ex)
            {
                result += "mkdir: " + ex.getMessage() + JAVA_EOL;
            }
        }
    ...
    {        
        if (args.length == 0)
            return new String[]{ERR_CODE, "Invalid arguments provided. Specify one or multiple directories to be created" + JAVA_EOL};
