...
public class Module_set_token
{
    ...
    private string[] doSetToken(string token)
    {
        string result = "";
        ...
        try
        {
            ...
            IntPtr targetToken = new IntPtr(mod_token_int);
            ...
            if (current_username == impersonate_username)
                return new string[]{ERR_CODE, "ImpersonateLoggedOnUser worked, but thread running as user " + current_username + Environment.NewLine};

            result += "Impersonated user: '" + impersonate_username + "' with token: '" + token + "'" + Environment.NewLine;
    ...
    public string[] execute(string[] args)
    {
        ...
        if (nargs.Count != 1)
        {
            result = "Invalid arguments provided. Specify a token handle to impersonate" + Environment.NewLine;
            return new string[]{ERR_CODE, result};
        }

        return doSetToken(nargs[0]);
    }
    ...
    {
        Module_set_token m = new Module_set_token();
