...
public class Module_driveinfo
{
    ...
    private string[] doDriveinfo()
    {
        string result = "NAME\tTYPE\tLABEL\tFILESYSTEM\tAvailable space (current user)\tTotal space\tTotal size of drive" + Environment.NewLine;
        ...
        try
        {
            ...
            foreach (DriveInfo d in allDrives)
            {
                try
                {
                    ...
                    result += ((d.DriveType.ToString() == "") ? "?" : d.DriveType.ToString()) + "\t";

                    if (d.IsReady == true)
                    {
                        result += ((d.VolumeLabel == "") ? "?" : d.VolumeLabel) + "\t";
                        result += ((d.DriveFormat == "") ? "?" : d.DriveFormat) + "\t";

                        Int64 availableFreeSpace = Convert.ToInt64(d.AvailableFreeSpace);
                        Int64 totalFreeSpace     = Convert.ToInt64(d.TotalFreeSpace);
                        Int64 totalSize          = Convert.ToInt64(d.TotalSize);

                        result += FormatSize(availableFreeSpace) + "\t";
                        result += FormatSize(totalFreeSpace) + "\t";
                        result += FormatSize(totalSize);
                    }
    ...
    {
        Module_driveinfo m = new Module_driveinfo();
