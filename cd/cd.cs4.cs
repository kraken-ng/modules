...
public class Module_cd
{
    ...
    private string[] doChangeDirectory(string new_directory)
    {
        string result = "";

        try
        {
            new_directory = normalizePath(new_directory);
            result = changeCWD(new_directory);
        }
        catch (Exception ex)
        {
    ...
    public string[] execute(string[] args)
    {
        ...
        if (args.Length != 1)
        {
            result = "Invalid arguments provided. Only one directory is allowed to be moved" + Environment.NewLine;
            return new string[]{ERR_CODE, result};
        }
