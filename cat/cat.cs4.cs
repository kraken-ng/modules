    ...
    private string[] doRead(string[] files)
    {
        ...
        foreach(string filepath in files) 
        {
            ...
            try
            {
                ...
                {
                    result += "cat: " + normalized_path + ": No such file, is a directory or permission denied" + Environment.NewLine;
                    continue;
                }

                result += File.ReadAllText(normalized_path, Encoding.UTF8);
            }
            catch(Exception ex)
            {
                retcode = ERR_CODE;
                result += "cat: failed on '" + normalized_path + "' : " + ex.Message + Environment.NewLine;
            }
        }
