        ...
        for (String infile : files)
        {
            try
            {
                ...
                if (input_file.exists() == false)
                    throw new Exception("can't perform stat on '" + infile + "': File or directory does not exist");

                if ((input_file.canRead() == false) || (input_file.canWrite() == false))
                    throw new Exception("can't perform stat on '" + infile + "': Permission denied");
                
                try
                {
                    if(input_file.isDirectory() && input_file.list().length != 0)
                        throw new Exception("directory: '" + infile + "': is not empty");

                    if (input_file.delete() == false)
                        throw new Exception("failed to delete: '" + infile + "'");
                }
                catch(SecurityException ex)
                {
                    throw new Exception("can't delete file '" + infile + "': access denied or failed");
                }
            }
            catch(Exception ex)
            {
                result += "rm: " + ex.getMessage() + JAVA_EOL;
            }
        }
