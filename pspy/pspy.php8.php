...
class Module_pspy
{
    ...
    private function processPIDStatusFile($pid)
    {
        ...
        if (@file_exists("/proc/$pid/status") === false)
            throw new Exception("Unexistent 'status' file on process id: $pid", 1);
        
        if (@is_readable("/proc/$pid/status") === false)
            throw new Exception("Unreadable 'status' file on process id: $pid", 1);
    ...
    private function parseProcessInfo($pinfo)
    {
        ...
        if (array_key_exists("new", $pinfo) !== false)
            if ($pinfo["new"] === false)
                $output .= "0" . "\t";
            else
                $output .= "1" . "\t";
    ...
    private function extractProcessInfo($pid)
    {
    ...
    private function filterProcesses($curr_processes, $files)
    {
        $new_processes = array();
        foreach ($files as $file)
        {
            ...
            $found = false;
            foreach ($curr_processes as $curr_process)
            {
                if ($curr_process["pid"] === $file)
                {
                    $found = true;
                    break;
                }
    ...
    private function scanProcesses()
    {
        ...
        foreach ($files as $file)
        {
            try
            {
                $processes[] = $this->extractProcessInfo($file);
            }
            catch (Exception $e)
            {
                continue;
    ...
    private function processExists($processes, $new_process)
    {
    ...
    private function doSpyProcesses($interval, $duration)
    {
        $result = "NEW\tUSER\tPID\tPPID\tVSZ\tRSS\tTTY\tSTART\tELAPSED\tCOMMAND" . PHP_EOL;
        try
        {
            ...
            while (time() < $end_time) {
                ...
                $new_processes = $this->scanProcesses();
                foreach ($new_processes as $new_process) {

                    if ($this->processExists($processes, $new_process) === false)
                    {
                        $new_process["new"] = $new;
            ...
            foreach ($processes as $process)
            {
                $result .= $this->parseProcessInfo($process) . PHP_EOL;
            }
        }
        catch (Exception $e)
        {
    ...
    public function execute($args)
    {
        ...
        try
        {
            ...
            if (sizeof($parsed_args) != 2)
                throw new Exception("Invalid arguments provided. Specify interval and duration");

            if ((is_numeric($parsed_args[0]) === false) || (is_numeric($parsed_args[1]) === false))
                throw new Exception("Invalid arguments provided. Interval and duration must be numeric");

            $interval = intval($parsed_args[0]);
            $duration = intval($parsed_args[1]);

            $result = $this->doSpyProcesses($interval, $duration);
        }
        catch (Exception $e)
        {
            $result = $e->getMessage() . PHP_EOL;
